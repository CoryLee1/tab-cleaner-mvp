...
一个基于 Chrome 扩展的标签页管理工具，使用 Shadow DOM 技术实现圆角卡片 UI。
...
- **`public/assets/background.js`**：Service Worker，监听插件图标点击，注入 content script
- **`public/assets/content.js`**：Content Script，创建 Shadow DOM 并渲染卡片
...
3. **模块类型**：`content.js` 使用 IIFE 格式，确保可以通过 `executeScript` 注入
